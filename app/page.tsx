...
export default function Home() {
  ...
  async function reserveStock(productId: string, warehouseId: string) {
    try {
  ...
  async function cancelReservation(reservationId: string) {
    try {
      const response = await fetch(`/api/reserve/${reservationId}/release`, {
        method: "POST",
      });
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: load dashboard data once on mount
  useEffect(() => {
    fetchProducts();
    fetchReservations();
  }, []);
  ...
  return (
    <main className="p-10 min-h-screen bg-background">
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-1">Total Stock: {item.totalStock}</p>

                      <p>Reserved: {item.reservedStock}</p>
      ...
      <div className="mt-12">
        <h2 className="text-3xl font-bold mb-6">Active Reservations</h2>
